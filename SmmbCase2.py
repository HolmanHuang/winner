...
def get_qty(bal_btc,bal_ltc,price):
    return min(min(bal_btc/price,bal_ltc)/5,0.5)
...
while True:
    ...
    if (len(existing_orders) > 0) and (abs(mid_price - coinut_last_price) < 0.00005) and (time.time()-last_order_time <480):
        continue
    ...
    for i in range(1, 2):
        ...
        #qty = 0.00001 #improved for case2
        qty = get_qty(float(bal['BTC']),float(bal['LTC']),mid_price) 
